...
import { bookingAPI, type TIBooking } from "../../../features/todos/todosAPI";
import { toast } from "sonner";

type UpdateBookingProps = {
    booking: TIBooking | null; //can be null if no todo is selected
};
...
const UpdateBooking = ({ booking }: UpdateBookingProps) => {
    ...
    useEffect(() => {
        if (booking) {
            setValue("bookingName", booking.bookingName);
            setValue("description", booking.description);
            setValue("userId", booking.userId);
            setValue("dueDate", booking.dueDate.slice(0, 10));
            setValue("isCompleted", booking.isCompleted);
        } else {
            reset();
        }
    ...
    const onSubmit: SubmitHandler<UpdateBookingInputs> = async (data) => {
        try {
            ...
            const response = await updateBooking({ ...data, id: booking.id })
